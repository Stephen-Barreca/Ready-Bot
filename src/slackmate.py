...
# constants
BOT_ID = os.environ.get('bot_id', 'U5CM069UH')
TOKEN = os.environ.get('token', 'xoxb-182714213969-zTOYpuGNt7XcL8zy1PsBpqGW')
AT_BOT = "<@{}>".format(BOT_ID)
EXAMPLE_COMMAND = "do"
BOT_NAME = 'slackmate'
COMMANDS = ['do', 'what', 'calculate', 'report']
...
def handle_command(command, channel):
    ...
    if command.startswith('do'):
        ...
        response = "The code is being worked on right now"
    ...
    for KEYWORD in KEYWORDS:
        if KEYWORD in message:
            ...
            if trigger_roll < KEYWORD_TRIGGER_PERCENT:
                ...
                print('The almighty Ran Dom has blocked this message')

    if response:
        slack_client.api_call("chat.postMessage", channel=channel, text=response, as_user=True)

def handle_report(command, channel):
    pass
